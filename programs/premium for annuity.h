...
float Premium(float pvUnitAnnualPayment, float forceOfMortality, float delta, float m, float a, float A, float a_10, float i, float age, float B, float c, float fudgeFactor) {
    float V = 1.0f/(1.0f+i);
    float S = 100000.0f;
    float b = 0.02f;
    ...
    float X=10000;
    float n_10=10;
    return (S*A)+X*pow(V,n_10)*SurvivalRateUltimate(10, age, A, B, c, fudgeFactor)*a_10/a;
}
